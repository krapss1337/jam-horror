...
namespace Michsky.LSS
{
    ...
    public class LSS_LoadingScreen : MonoBehaviour
    {
        ...
        void Start()
        {
            if (enableRandomHints && hintsText != null && hintList.Count > 0) { hintsText.text = GetRandomHint(); StartCoroutine("ProcessRandomHint"); }
            else if (!enableRandomHints && hintsText != null && hintList.Count > 0) { hintsText.text = hintList[currentHintIndex]; }

            if (enableRandomImages && imageList.Count > 0) { ProcessRandomImages(); }
        ...
        {
            if (hintList.Count == 0) { return null; }
            else
            {
        ...
        public static int GetRandomUniqueValue(int currentValue, int minValue, int maxValue)
        {
            ...
            while (currentValue == value) { value = UnityEngine.Random.Range(minValue, maxValue); }
            return value;
        }
    }
}